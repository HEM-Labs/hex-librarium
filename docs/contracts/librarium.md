...
External projects should usually run the published initializer image as a one-shot Compose service.

Containers can copy the initializer script into an image and use it as an entrypoint or call it before launching the project-specific process:
...
`diffusion/checkpoint` is for bundled primary diffusion model checkpoints. These are files a UI loads as the main generation model, often containing some combination of denoiser, text encoder, VAE, config, and metadata. Examples include Stable Diffusion 1.x, SDXL, SD3, FLUX packaged checkpoints, and
Stable Video Diffusion checkpoints.
